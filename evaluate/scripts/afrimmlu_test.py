...
AFRIMMLU_PATH = "./evaluate/scripts/data/afrimmlu"
...
def read_afrimmlu(split: str = "test",
    ...
    for config in get_dataset_config_names(AFRIMMLU_PATH):
        ds = load_dataset(AFRIMMLU_PATH,
                          config,
                          split=split)

        for row in ds:
            if row["subject"] != "elementary_mathematics":
                continue
...
def main(
    args,
    afrimmlu_dir: str = AFRIMMLU_PATH,
    is_bf16: bool = True,
    save_dir: str  = None,
):
    ...
    if save_dir is None:
        save_dir = f"./results/afrimmlu/{model_name}"

    Path(save_dir).mkdir(parents=True, exist_ok=True)
    
    afrimmlu_ds = read_afrimmlu()
    if args.lang_only is None:
        langs = afrimmlu_ds.keys()
    else:
    ...
    with open(Path(save_dir) / f"afrimmlu_eval_bs{batch_size}.csv", 'w', newline='') as file:
